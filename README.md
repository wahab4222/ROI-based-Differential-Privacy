...
The revised Zenodo release provides the immutable archival snapshot of the software and associated reproducibility materials. The GitHub repository contains the current revised code and lightweight supporting artifacts. The trained InceptionV3 checkpoint used for the qualitative visualization is hosted in the Zenodo release because of its file size and is not duplicated in this GitHub repository.
...
The `visualization/` directory contains the lightweight files used to reproduce the qualitative visualization reported in the revised manuscript.
...
| `final_metrics.csv` | Contains the final evaluation metrics for the selected experiment, including the test performance associated with the visualization model. |
| `test_predictions.csv` | Contains held-out test predictions and class probabilities used by the visualization script to select representative correctly classified AD, CN, and MCI samples. |

### Model checkpoint

The trained checkpoint required by the visualization script is:

```text
inceptionv3_roi_layer_beta2p0_best_val_global_model.pt
```

The checkpoint is approximately 83 MB and is therefore **not stored directly in this GitHub repository**. It is archived as part of the immutable revised Zenodo release:

**https://doi.org/10.5281/zenodo.22093150**

To reproduce the qualitative figure, download the checkpoint from Zenodo and place it in the location expected by `generate_qualitative_visualization.py`, or update `CKPT_PATH` in the script to point to the downloaded file.
...
The OASIS MRI images themselves are not redistributed in this repository or in the software release. Users must obtain the dataset independently and update the dataset path in the visualization script before reproducing the figure.
